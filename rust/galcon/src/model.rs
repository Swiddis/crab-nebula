...
impl Galaxy {
    ...
    pub fn update(self: &mut Galaxy, command: &ServerMessage) {
        match command {
            ...
            ServerMessage::User(user) => _ = self.users.insert(user.id, user.clone()),
            ServerMessage::Planet(planet) => _ = self.planets.insert(planet.id, planet.clone()),
            ServerMessage::Fleet(fleet) => _ = self.fleets.insert(fleet.id, fleet.clone()),
